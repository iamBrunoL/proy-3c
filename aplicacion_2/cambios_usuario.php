...
<body>
    ...
    <div id="right-panel" class="right-panel">
		...
		<form method="POST" action="<?php htmlspecialchars($_SERVER["PHP_SELF"]);?>">
			...
			<input type="submit" class="btn btn-primary" name ="buscar" id="buscar" value="Buscar numero de usuario!!!"/>
			<br/><hr/>
            <br><br>
			<?php
				if(isset($_REQUEST['buscar'])){
					$id=isset($_REQUEST['id']) ? $_REQUEST['id'] : null;
					$query = $con->prepare('select * FROM registro where id = :id');
					...
					if($query -> rowCount() > 0){
						...
						echo 	'<label>Id de usuario:</label>'.
									...
									'<input type="text" lang="es" href="qa-html-language-declarations.es".
										name="nombre" value ="'.$row['nombre'].'"/><br/>'.
                                    
									'<label>Correo:</label>'.
									'<input type="text" lang="es" href="qa-html-language-declarations.es".
										name="correo" value ="'.$row['correo'].'"/><br/>'.
                                    
									'<label>Contraseña del usuario:</label>'.
									'<input type="text" lang="es" href="qa-html-language-declarations.es".
										name="clave" value ="'.$row['clave'].'"/><br/>'.
                ...
                if(isset($_REQUEST['cambiar'])){
					...
					$sql = "UPDATE registro SET id=?, nombre=?, correo=?, clave=? WHERE id=?";
					$stmt= $con->prepare($sql);
					$stmt->execute([$id, $nombre, $correo, $clave,$id]);
